...
// 📦 HÀM DÀNH RIÊNG CHO TRANG AUDIO
  const renderNFTCard = (nft: NFT) => (
    <Link href={`/explore/${nft.id}`} key={`audio-card-${nft.id}`} className="group flex flex-col rounded-3xl bg-gray-800 border border-gray-700 overflow-hidden hover:-translate-y-2 hover:shadow-[0_10px_40px_rgba(37,99,235,0.2)] transition-all duration-300">
      
      {/* Khu vực hiển thị Âm thanh */}
      <div className="aspect-square bg-gray-700 relative overflow-hidden flex items-center justify-center bg-gray-900 group">
        {nft.coverImage ? (
          <img src={resolveIpfsUrl(nft.coverImage)} alt="Audio Cover" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 ease-in-out opacity-70 group-hover:opacity-100" />
        ) : (
      ...
      {/* Khu vực thông tin giữ nguyên */}
      <div className="p-5 flex flex-col flex-grow">
        <div className="mb-4">
          <h3 className="font-bold text-lg text-white group-hover:text-green-400 transition-colors line-clamp-1">{nft.name}</h3>
        ...
        <div className="mt-auto pt-4 border-t border-gray-700 flex justify-between items-end">
          ...
          <button className="text-sm font-bold bg-gray-700/50 text-green-400 px-4 py-2 rounded-xl group-hover:bg-green-600 group-hover:text-white transition-colors"> Xem </button>
        </div>
      </div>
    </Link>
  );
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-10">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight mb-4 flex items-center gap-4">
            <Music size={40} className="text-green-400" /> Âm thanh Độc quyền
          </h1>
          <p className="text-gray-400 text-lg max-w-2xl">
            Khám phá bộ sưu tập Âm thanh độc bản được lưu trữ trên Blockchain.
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center"> 
            ...
            <h3 className="text-2xl font-bold text-gray-300">Chưa có Hình ảnh nào</h3>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
