...
export default function SettingsPage() {
  ...
  const fetchProfile = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`/api/profile?wallet=${currentAccount}`); // Sửa lại query cho khớp API
      if (response.ok) {
  ...
  const handleSave = async () => {
    ...
    try {
      // --- BƯỚC MỚI: BẮT CHỮ KÝ BẢO MẬT ---
      const provider = new ethers.BrowserProvider((window as any).ethereum);
      const signer = await provider.getSigner();
      const message = `Tôi xác nhận cập nhật hồ sơ trên chợ NFT cho ví: ${currentAccount}`;
      
      // Popup con cáo sẽ hiện lên ở đây
      ...
      // BƯỚC 2: GỌI API PROFILE (KÈM THEO CHỮ KÝ)
      ...
      const payload = {
        ...
        signature: signature // <-- Truyền chữ ký vào payload để backend kiểm duyệt
      };
      ...
      // Bắt lỗi nếu người dùng bấm "Reject" trên MetaMask
  ...
  // ... (Toàn bộ phần return HTML giao diện giữ nguyên không đổi)
  return (
    <div className="min-h-screen bg-[#0e111a] text-white py-12 px-4 md:px-8">
      {/* HTML cũ của ông */}
      <div className="max-w-4xl mx-auto">
        <button onClick={() => router.back()} className="inline-flex items-center gap-2 text-gray-400 hover:text-white mb-6 transition-colors group cursor-pointer">
